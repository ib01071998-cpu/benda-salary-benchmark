...
# הגדרות מערכת
# -------------------------------------------------
st.set_page_config(page_title="מערכת בנצ'מארק שכר – Master PRO", layout="wide")
load_dotenv()
...
צור טבלת בנצ'מארק שכר מדויקת לתפקיד "{job_title}" בישראל {exp_text} לשנת 2025.

הוראות חובה:
1. הצג טווחים ריאליים לכל רכיב (יחס מקסימום/מינימום ×1.2–×1.5).
2. הצג ממוצע מחושב אמין.
3. הצג שלוש רמות תגמול: בסיסית, בינונית, גבוהה.
4. הצג מנגנוני תגמול מפורטים עם דוגמה מספרית, מדרגות ותקרה.
5. הצג אחוזי עלות שכר כוללת (%).

מיפוי אוטומטי של סוגי רכיבים:
- "עמלות", "יעדים", "אחוזים", "תגמול משתנה" → הצג באחוזים (%)
- "שכר", "בסיס", "בונוס", "מענק", "אש\"ל", "טלפון", "ציוד", "רווחה" → ₪
- "קרן השתלמות", "פנסיה", "ביטוח מנהלים" → % מהשכר
- "רכב חברה" → שווי שוק ₪ + זקיפת שווי ₪ (כ־3,000 ₪ לחודש)
- "אופציות", "RSU", "מניות" → % או ₪ שנתי

אם חסרים רכיבי שכר רלוונטיים למשרה — הוסף אותם אוטומטית.

⚠️ ברכיב "רכב חברה":
- הצג את שווי השוק של הרכב (₪)
- הצג שלושה דגמים תואמים
- הצג את זקיפת השווי לעובד (לא עלות למעסיק).

בסוף הטבלה הוסף שורה:
"סה״כ עלות מעסיק ממוצעת (₪)" — סכום כל הרכיבים.

הטבלה בעברית מלאה, ברורה, מימין לשמאל בלבד.
"""
...
# ממשק המשתמש
# -------------------------------------------------
st.title("💼 מערכת בנצ'מארק שכר – Master PRO")
...
if run:
    if not job.strip():
        ...
        with st.spinner("🧠 מפיק בנצ'מארק מדויק ומפורט..."):
