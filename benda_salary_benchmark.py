...
# הגדרות מערכת
# -------------------------------------------------
st.set_page_config(page_title="מערכת בנצ'מארק שכר - גרסה ישראלית מקצועית", layout="wide")
load_dotenv()
...
צור טבלת שכר מקצועית ומפורטת לתפקיד "{job_title}" בישראל לשנת 2025,
בהתבסס על נתוני אמת משוק העבודה הישראלי (AllJobs, Drushim, Globes, Calcalist, Bizportal)
ועל נתוני חברות יבוא, לוגיסטיקה, תעשייה ואלקטרוניקה הדומות לבנדא מגנטיק.
...
- רמות שכר: בסיסית / בינונית / גבוהה
- ממוצע שוק (₪)
- **מנגנון תגמול מפורט כפי שמקובל בישראל בפועל**, לדוגמה:
  - עמלות: 5% מהמכירות נטו עד תקרה של 8,000 ₪
  - בונוס שנתי: עד 10% מהשכר השנתי על פי יעדים
  - רכב חברה: קבוצת רכב 4 בשווי שוק 150,000 ₪ עם זקיפת שווי של כ-3,000 ₪ לחודש
  - קרן השתלמות: 7.5% מהמעסיק
  - שעות נוספות: לפי תעריף 125%-150% מעבר לשעות התקן
- עלות מעסיק ממוצעת (₪)
- אחוז מעלות השכר הכוללת (%)

⚠️ רכיב "רכב חברה":
ציין את שווי השוק של הרכב (₪), שלושה דגמים תואמים לשווי זה,
ובעבור עלות המעסיק הצג את **זקיפת השווי** (לדוג׳ 3,000 ₪ לחודש לקבוצה 4).

הפלט יהיה **אך ורק טבלה אחת** בפורמט הבא:
...
with col2:
    if st.button("🗑️ נקה היסטוריה"):
        ...
        st.success("היסטוריית החיפושים נוקתה בהצלחה ✅")
