...
השתמש בנתונים אמיתיים ובמנגנוני השכר המקובלים במשק הישראלי בלבד.

לכל רכיב חובה להציג ערכים ביחידות הרלוונטיות לו:
- שכר בסיס, בונוסים, אש"ל – ₪ לחודש
- עמלות – באחוזים מהמכירות או מהרווח הגולמי
- שעות נוספות – תעריף ₪ לשעה נוספת
- ביטוחים וקרנות – באחוזים מהשכר
- רכב חברה – שווי שוק ₪ וזקיפת שווי ₪ לחודש
- הבראה, מתנות, רווחה – ₪ שנתי ממוצע

ברכיבי השכר המשתנים (עמלות, בונוסים, מענקים) יש להציג בעמודות:
- רמה בסיסית
- רמה בינונית
- רמה גבוהה
כל אחת עם טווחים באחוזים או סכומים.

בנוסף, לכל רכיב הצג:
- ממוצע שוק (₪)
- מנגנון תגמול מפורט ומציאותי כפי שמקובל במשק הישראלי
- עלות מעסיק ממוצעת (₪)
- אחוז מעלות השכר הכוללת (%)

ברכיב "רכב חברה":
- הצג את שווי השוק של הרכב הרלוונטי לתפקיד (₪)
- הצג שלושה דגמים אופייניים בשווי זה
- בעמודת עלות מעסיק הצג **זקיפת שווי רכב ממוצעת בישראל** (≈3,000 ₪ לחודש)

בסוף הדוח הוסף סיכום מקצועי וברור הכולל:
- שכר ברוטו ממוצע כולל (₪)
- עלות מעסיק כוללת (₪) לפי יחס 1.35× שכר הברוטו + עלויות נוספות
- הערכת שווי כוללת להטבות (₪)
"""
...
# ממשק המשתמש
# -------------------------------------------------
st.title("💼 מערכת בנצ'מארק שכר – גרסת Ultimate ישראלית (מותאם לשוק)")
...
if run:
    if not job.strip():
        ...
        <div class="summary-box">
          ...
          <div class="summary-line">⚖️ <span class="summary-value">מנגנוני תגמול:</span> מוצגים בהתאם למקובל בישראל – עמלות, בונוסים, שעות נוספות, וזקיפת שווי.</div>
        </div>
        """, unsafe_allow_html=True)
