...
# הגדרות מערכת
# -------------------------------------------------
st.set_page_config(page_title="מערכת בנצ'מארק שכר – גרסת Precision+", layout="wide")
load_dotenv()
...
שלב בין הנתונים המספריים שמצאת למידע האקטואלי המקובל בישראל בפועל.

⏩ הנחיות תצוגה לפי סוג רכיב:
- אם מדובר ברכיב מבוסס **עמלות, אחוזים, תגמול יחסי או בונוס מכירות** — הצג את כל הערכים באחוזים (%).
- אם מדובר ברכיב קבוע כמו **שכר בסיס, אש"ל, הבראה, בונוס שנתי, ציוד, טלפון, ביטוחים וכו׳** — הצג את כל הערכים בשקלים (₪).
- אם מדובר ב**שעות נוספות**, הצג תשלום לשעה נוספת (₪ לשעה) בלבד, ולא מכסת שעות חודשית.

לכל רכיב הצג:
- שלוש רמות תגמול בעמודות (בסיסית, בינונית, גבוהה)
- טווח ערכים (₪ או % לפי סוג הרכיב)
- ממוצע שוק (₪ או %)
- מנגנון תגמול מפורט **בהתאם למקובל בישראל בפועל**
- עלות מעסיק ממוצעת (₪)
- אחוז מעלות השכר הכוללת (%)

⚠️ ברכיב "רכב חברה":
- הצג את **שווי השוק של הרכב** (₪)
- הצג שלושה דגמים תואמים לשווי זה
- בעמודת "עלות מעסיק ממוצעת" הצג את **זקיפת השווי** לעובד (הסכום החודשי המחויב במס, לדוג׳: 3,000 ₪ לחודש עבור רכב קבוצה 4).
...
# ממשק המשתמש
# -------------------------------------------------
st.title("💼 מערכת בנצ'מארק שכר – גרסת Precision+")
