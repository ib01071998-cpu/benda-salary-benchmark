...
בהתאם לשוק העבודה הישראלי.

הנחיות מחמירות:
- השתמש ביחידות הרלוונטיות לכל רכיב: ₪ לחודש, ₪ לשנה, אחוזים (%), או מספר ימי חופשה.
- הצג טווחים אמיתיים בכל שורה.
- מנגנוני תגמול חייבים לשקף את **הנורמות המקובלות במשק הישראלי** בפועל (לא נוסח תאורטי).
- דוגמה: עמלות מכירה 5% מהמכירות נטו עד תקרה של 8,000 ₪; בונוס רבעוני בגובה משכורת אחת; קרן השתלמות עד 7.5% מהמעסיק.
- יש לכלול שלוש רמות תגמול באותה שורה (בסיסית, בינונית, גבוהה).
- בסעיף "רכב חברה": בעמודת "עלות מעסיק ממוצעת" הצג **זקיפת שווי** לעובד (לא עלות המעסיק בפועל).

רשימת רכיבי שכר חובה:
...
לכל רכיב הצג:
- טווח רלוונטי ביחידות הנכונות (₪, %, ימים וכו')
- שלוש רמות תגמול בעמודות (בסיסית / בינונית / גבוהה)
- ממוצע שוק (₪)
- מנגנון תגמול מפורט ומציאותי בישראל (לא נוסחה כללית)
- עלות מעסיק ממוצעת (₪ או אחוז זקיפת שווי)
- אחוז מעלות השכר הכוללת (%)

בסוף הדוח הוסף סיכום מקצועי הכולל:
...
"""
    r = client.chat.completions.create(
        model="gpt-4-turbo",
        messages=[
            {"role": "system", "content": "אתה אנליסט שכר בכיר בישראל. הפלט הוא טבלה אחת בלבד בעברית, עם יחידות ריאליות ומנגנוני תגמול אמיתיים מהשוק."},
